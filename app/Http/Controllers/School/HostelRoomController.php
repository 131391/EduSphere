...
class HostelRoomController extends TenantController
{
    ...
    public function index(Request $request)
    {
        ...
        $transformer = function ($room) {
            $amenities = [];
            if (($room->ac?->value ?? $room->ac) === YesNo::Yes->value) $amenities[] = 'AC';
            if (($room->cooler?->value ?? $room->cooler) === YesNo::Yes->value) $amenities[] = 'Cooler';
            if (($room->fan?->value ?? $room->fan) === YesNo::Yes->value) $amenities[] = 'Fan';
            ...
            return [
                ...
                'ac'              => $room->ac?->value ?? $room->ac ?? YesNo::No->value,
                'cooler'          => $room->cooler?->value ?? $room->cooler ?? YesNo::No->value,
                'fan'             => $room->fan?->value ?? $room->fan ?? YesNo::No->value,
                'amenities'       => $amenities,
        ...
        $callback = function () use ($query) {
            ...
            $query->orderBy('created_at', 'desc')->cursor()->each(function ($room) use ($file) {
                fputcsv($file, [
                    ...
                    (($room->ac?->value ?? $room->ac) === YesNo::Yes->value) ? 'Yes' : 'No',
                    (($room->cooler?->value ?? $room->cooler) === YesNo::Yes->value) ? 'Yes' : 'No',
                    (($room->fan?->value ?? $room->fan) === YesNo::Yes->value) ? 'Yes' : 'No',
    ...
    public function store(Request $request)
    {
        try {
            ...
            return response()->json(['success' => true, 'message' => 'Room added successfully.', 'data' => $room]);
        } catch (\Illuminate\Validation\ValidationException $e) {
            return response()->json(['success' => false, 'message' => 'Validation failed.', 'errors' => $e->errors()], 422);
        } catch (\Exception $e) {
            return response()->json(['success' => false, 'message' => 'Failed to add room: ' . $e->getMessage()], 500);
        }
    }
    ...
    {
        return redirect()->route('school.hostel.rooms.index', ['export' => 'csv']);
    }
}
