...
class AdmissionFeeController extends TenantController
{
    ...
    public function index(Request $request)
    {
        ...
        $query = AdmissionFee::where('school_id', $schoolId)->with('class');
        ...
        $assignedClassIds  = AdmissionFee::where('school_id', $schoolId)->pluck('class_id');
        $unassignedClasses = $allClasses->whereNotIn('id', $assignedClassIds)->values();

        return view('school.settings.admission-fee.index', [
    ...
    public function store(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            'class_id' => [
                ...
                Rule::unique('admission_fees')->where('school_id', $schoolId),
            ],
            'amount' => 'required|numeric|min:0|max:999999.99',
        ], [
            'class_id.exists'  => 'The selected class is not valid for this school.',
            'class_id.unique'  => 'An admission fee for this class already exists.',
    ...
    public function update(Request $request, AdmissionFee $admissionFee)
    {
        ...
        $admissionFee->update(['amount' => $request->amount]);

        return response()->json([
            'success' => true,
            'message' => 'Admission fee updated successfully.',
            'data'    => $admissionFee->fresh()->toArray(),
        ]);
    }
    ...
    public function destroy(AdmissionFee $admissionFee)
    {
        ...
        return response()->json(['success' => true, 'message' => 'Admission fee deleted.']);
    }

    private function getStats(int $schoolId): array
    {
        return [
            'total_configurations'  => AdmissionFee::where('school_id', $schoolId)->count(),
            'average_admission_fee' => number_format(AdmissionFee::where('school_id', $schoolId)->avg('amount') ?? 0, 2),
            'unique_classes'        => AdmissionFee::where('school_id', $schoolId)->distinct('class_id')->count('class_id'),
        ];
    }
}
