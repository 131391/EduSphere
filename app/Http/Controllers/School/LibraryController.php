...
class LibraryController extends TenantController
{
    ...
    public function index(Request $request)
    {
        ...
        $allowedSorts = ['title', 'author', 'quantity', 'available_quantity', 'price'];
    ...
    public function issues(Request $request)
    {
        ...
        $direction = $request->input('direction', 'asc') === 'desc' ? 'desc' : 'asc';
        $allowedSorts = ['issue_date', 'due_date', 'id'];
        if (in_array($sort, $allowedSorts, true)) {
            $query->orderBy($sort, $direction);
        }
    ...
    public function searchStaff(Request $request)
    {
        ...
        $term    = '%' . addcslashes((string) $request->input('q', ''), '%_\\') . '%';
        $results = Staff::where('school_id', $this->getSchoolId())
            ->where(fn($query) => $query
    ...
    public function renew(Request $request, BookIssue $issue)
    {
        $this->ensureSchoolActive();
        $this->authorizeTenant($issue);
        Gate::authorize('manageIssue', $issue);
        ...
        try {
            ...
            Log::error('Failed to renew library book.', ['school_id' => $this->getSchoolId(), 'issue_id' => $issue->id, 'exception' => $e]);
            return $request->wantsJson()
