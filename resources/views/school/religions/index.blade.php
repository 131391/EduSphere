...
<div x-data="religionManagement()">
        ...
        $tableActions = [
            [
                ...
                'onclick' => function($row) {
                    ...
                    return "window.dispatchEvent(new CustomEvent('open-edit-religion', { detail: JSON.parse(atob('$encoded')) }))";
                },
                'title' => 'Edit',
            ],
            [
                ...
                'onclick' => function($row) {
                    return "window.dispatchEvent(new CustomEvent('open-delete-religion', { detail: { id: " . $row->id . ", name: '" . addslashes($row->name) . "' } }))";
                },
                'title' => 'Delete',
            ],
        ];
    ...
    <x-modal name="religion-modal" alpineTitle="editMode ? 'Edit Religion' : 'Create New Religion'" maxWidth="md">
        <form @submit.prevent="submitForm" method="POST" class="p-0" novalidate>
            ...
            <div class="px-8 py-8">
                <div>
                    <label class="block text-sm font-semibold text-gray-800 mb-1.5 ml-1">Religion Name <span class="text-red-500">*</span></label>
            ...
            <div class="px-8 py-6 bg-gray-50/50 flex items-center justify-end gap-3 rounded-b-lg border-t border-gray-100">
                ...
                >
                    ...
                    <span x-text="editMode ? (submitting ? 'Updating' : 'Save Changes') : (submitting ? 'Creating' : 'Create Religion')"></span>
                </button>
            </div>
        </form>
    </x-modal>
...
document.addEventListener('alpine:init', () => {
    Alpine.data('religionManagement', () => ({
        ...
        async confirmDelete(religion) {
            if (window.confirm(`Are you sure you want to delete the religion "${religion.name}"?`)) {
                try {
