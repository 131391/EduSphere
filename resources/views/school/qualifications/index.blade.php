...
@section('content')
    <div x-data="Object.assign(ajaxDataTable({
        fetchUrl: '{{ route('school.qualifications.fetch') }}',
        defaultSort: 'name',
    ...
    }), qualificationManagement())" class="space-y-6" @close-modal.window="if ($event.detail === 'qualification-modal') { resetForm(); }">

        <!-- Statistics Cards -->
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <x-stat-card label="Total Qualifications" :value="$stats['total']" icon="fas fa-graduation-cap" color="indigo" alpine-text="stats.total" />
        </div>

        <!-- Header Section -->
        <x-page-header title="Qualifications" description="Manage education and professional qualifications" icon="fas fa-graduation-cap">
            ...
            <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
                <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    <div class="flex-1 flex flex-col md:flex-row md:items-center gap-4">
                        <h2 class="text-lg font-semibold text-gray-800 dark:text-white">Qualifications List</h2>
                        <x-table.search placeholder="Search qualifications..." />
                    </div>
            ...
            <div class="overflow-x-auto relative ajax-table-wrapper">
                ...
                <table class="w-full text-left border-collapse">
                    <thead class="bg-gray-50/50 dark:bg-gray-700/50 border-b border-gray-100 dark:border-gray-700">
                        <tr>
                            <x-table.sort-header column="name" label="Qualification" sort-var="sort" direction-var="direction" />
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-700 transition-opacity duration-150" x-cloak :class="loading && rows.length &gt; 0 ? 'opacity-50' : 'opacity-100'">
                        ...
                        <x-table.empty-state :colspan="3" icon="fas fa-graduation-cap" message="No qualifications found." />
                    </tbody>
                </table>
            </div>
        ...
        <x-modal name="qualification-modal" alpineTitle="editMode ? 'Edit Qualification' : 'Create New Qualification'" maxWidth="2xl">
            <form @submit.prevent="submitForm()" method="POST" novalidate class="p-1">
                ...
                <x-slot name="footer">
                    <button type="button" @click="$dispatch('close-modal', 'qualification-modal')" class="btn-premium-cancel px-10">
                        Cancel
                    </button>
                    ...
                    confirmDelete(qualification) {
                        ...
                        window.dispatchEvent(new CustomEvent('open-confirm-modal', {
                            detail: {
                                ...
                                message: `Are you sure you want to delete "${qualification.name}"? This action cannot be undone.`,
                                callback: async () => {
                                    try {
