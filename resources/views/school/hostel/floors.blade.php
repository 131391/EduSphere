...
<div x-data="ajaxDataTable({
    endpoint: '{{ route('school.hostel.floors.fetch') }}',
    storeEndpoint: '{{ route('school.hostel.floors.store') }}',
    updateEndpoint: '{{ route('school.hostel.floors') }}',
    entityName: 'Floor'
})" class="p-6">
    <!-- Header -->
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">Hostel Floors</h1>
            <p class="text-gray-600">Organize hostels by floors</p>
        </div>
    ...
    <x-data-table>
        <x-slot name="head">
            <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Floor Name</th>
            <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Hostel</th>
            <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Rooms</th>
            <th class="px-6 py-4 text-right text-xs font-semibold text-gray-500 uppercase tracking-wider">Actions</th>
        </x-slot>
        <x-slot name="body">
            <template x-for="item in items" :key="item.id">
                <tr class="hover:bg-gray-50 transition-colors">
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900" x-text="item.floor_name"></td>
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="flex items-center text-sm text-gray-600">
                            <i class="fas fa-hotel mr-2 text-gray-400"></i>
                            <span x-text="item.hostel_name"></span>
                        </div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                        <span class="px-2 py-1 bg-indigo-50 text-indigo-600 rounded-md" x-text="item.room_count + ' Rooms'"></span>
                    </td>
    ...
    <!-- Modal -->
    <x-modal x-show="showModal" @close="showModal = false" :title="editMode ? 'Edit Floor' : 'Add New Floor'">
        <form @submit.prevent="saveItem" class="space-y-4">
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Select Hostel <span class="text-red-500">*</span></label>
                <select x-model="formData.hostel_id" required 
            ...
            <div>
                ...
                <input type="text" x-model="formData.floor_name" required placeholder="e.g. Ground Floor, 1st Floor" 
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all">
            </div>
            <div class="flex justify-end gap-3 pt-4">
                ...
                <button type="submit" class="px-6 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-all shadow-md shadow-indigo-200" 
                    :disabled="submitting" x-text="submitting ? 'Saving...' : 'Save Floor'"></button>
            </div>
        </form>
    </x-modal>
</div>
