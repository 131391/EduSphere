...
@section('content')
    <div x-data="Object.assign(ajaxDataTable({
        fetchUrl: '{{ route('school.hostel.assignments.fetch') }}',
                        ...
                        <tr class="hover:bg-gray-50/50 dark:hover:bg-gray-800/50 transition-colors">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <div class="flex items-center justify-center gap-2">
                                    ...
                                    <button @click="quickAction('{{ route('school.hostel.assignments.destroy', $row['id']) }}', 'Delete', 'DELETE', 'Are you sure you want to remove this assignment?')" class="w-8 h-8 rounded-lg bg-rose-50 text-rose-600 flex items-center justify-center hover:bg-rose-100 transition-colors" title="Delete"><i class="fas fa-trash-alt text-xs"></i></button>
                                </div>
                            </td>
                        </tr>
                        @endforeach
                            ...
                            <tr class="hover:bg-gray-50/50 dark:hover:bg-gray-800/50 transition-colors">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        ...
                                        <button @click="quickAction(`/school/hostel/assignments/${row.id}`, 'Delete', 'DELETE', 'Are you sure you want to remove this assignment?')" class="w-8 h-8 rounded-lg bg-rose-50 text-rose-600 flex items-center justify-center hover:bg-rose-100 transition-colors"><i class="fas fa-trash-alt text-xs"></i></button>
                                    </div>
                                </td>
                            </tr>
                        </template>
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div class="space-y-2">
                                ...
                                <div :class="errors.hostel_id ? 'border border-red-500 rounded-xl' : 'border border-slate-200 dark:border-gray-600 rounded-xl'">
                                    <select name="hostel_id" x-model="formData.hostel_id" @change="loadFloors(); clearError('hostel_id')"
                                        class="no-select2 w-full bg-white dark:bg-gray-700 border-0 rounded-xl py-3 px-4 text-sm font-bold text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-teal-500/20 transition-all shadow-sm">
                            ...
                            <div class="space-y-2">
                                ...
                                <div :class="errors.hostel_floor_id ? 'border border-red-500 rounded-xl' : 'border border-slate-200 dark:border-gray-600 rounded-xl'">
                                    <select name="hostel_floor_id" x-model="formData.hostel_floor_id"
                                        ...
                                        class="no-select2 w-full bg-white dark:bg-gray-700 border-0 rounded-xl py-3 px-4 text-sm font-bold text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-teal-500/20 transition-all shadow-sm disabled:opacity-50 disabled:bg-gray-50">
                            ...
                            <div class="space-y-2">
                                ...
                                <div :class="errors.hostel_room_id ? 'border border-red-500 rounded-xl' : 'border border-slate-200 dark:border-gray-600 rounded-xl'">
                                ...
                                <template x-if="errors.hostel_room_id && errors.hostel_room_id[0]">
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="grid grid-cols-2 gap-4 bg-slate-50/50 dark:bg-gray-700/50 p-5 rounded-2xl border border-slate-100 dark:border-gray-600 shadow-inner">
                                ...
                                <div class="space-y-2">
                                    ...
                                    <div :class="errors.starting_month ? 'border border-red-500 rounded-xl' : 'border border-slate-200 dark:border-gray-600 rounded-xl'">
                                    ...
                                    <template x-if="errors.starting_month && errors.starting_month[0]">
        ...
        <script>
            function bedAssignmentManagementData() {
                return {
                    ...
                    async loadMonths() {
                        try {
                            const response = await fetch('{{ route('school.hostel.assignments.get-months') }}');
                    ...
                    searchStudents() {
                        ...
                        this.searchTimeout = setTimeout(async () => {
                            try {
                                const response = await fetch('{{ route('school.hostel.assignments.search-students') }}', {
                                    method: 'POST',
                                    headers: {
                    ...
                    async loadFloors(targetFloorId = null) {
                        ...
                        try {
                            const response = await fetch('{{ route('school.hostel.assignments.get-floors') }}', {
                                method: 'POST',
                                headers: {
                                    'Content-Type': 'application/json',
                                    'X-CSRF-TOKEN': '{{ csrf_token() }}',
                    ...
                    async loadRooms(targetRoomId = null) {
                        ...
                        try {
                            const response = await fetch('{{ route('school.hostel.assignments.get-rooms') }}', {
                                method: 'POST',
                                headers: {
                                    'Content-Type': 'application/json',
                                    'X-CSRF-TOKEN': '{{ csrf_token() }}',
                    ...
                    async submitForm() {
                        ...
                        try {
                            const url = this.editMode 
                                ? `/school/hostel/assignments/${this.assignmentId}` 
                                : '{{ route('school.hostel.assignments.store') }}';
