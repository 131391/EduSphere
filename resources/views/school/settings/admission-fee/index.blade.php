...
<div x-data="admissionFeeManager()">
        ...
        $tableActions = [
            [
                ...
                'onclick' => function($row) {
                    ...
                    return "window.dispatchEvent(new CustomEvent('open-edit-admission-fee', { detail: JSON.parse(atob('$encoded')) }))";
                },
                'title' => 'Edit',
            ],
            [
    ...
    <div x-on:open-edit-admission-fee.window="openEditModal($event.detail)" 
         x-on:open-delete-admission-fee.window="confirmDelete($event.detail)">
        <x-data-table 
...
document.addEventListener('alpine:init', () => {
    Alpine.data('admissionFeeManager', () => ({
        ...
        async confirmDelete(fee) {
            if (window.confirm(`Are you sure you want to delete the admission fee for "${fee.name}"?`)) {
                try {
