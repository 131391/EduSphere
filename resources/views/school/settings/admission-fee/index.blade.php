...
<div x-data="admissionFeeManager()">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-4 mb-6 border border-emerald-100/50">
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Configure one-time admission fees applied during the enrollment process</p>
            </div>
        ...
        $tableActions = [
            [
                ...
                'onclick' => function($row) {
                    ...
                    return "window.dispatchEvent(new CustomEvent('open-edit-admission-fee', { detail: $data }))";
                },
                'title' => 'Edit',
            ],
            [
                ...
                'onclick' => function($row) {
                    return "window.dispatchEvent(new CustomEvent('open-delete-admission-fee', { detail: { id: " . $row->id . ", name: '" . addslashes($row->class->name ?? 'N/A') . "' } }))";
                },
                'title' => 'Delete',
            ],
        ];
    ...
    <div>
        <x-data-table 
            ...
            empty-message="No admission fees configured"
    ...
    <!-- Modal -->
    <x-modal name="admission-fee-modal" alpineTitle="editMode ? 'Edit Admission Rate' : 'Configure New Admission Rate'" maxWidth="md">
...
document.addEventListener('alpine:init', () => {
    Alpine.data('admissionFeeManager', () => ({
        editMode: false,
        feeId: null,
        submitting: false,
        errors: {},
        ...
        init() {
            window.addEventListener('open-edit-admission-fee', (e) => this.openEditModal(e.detail));
            window.addEventListener('open-delete-admission-fee', (e) => this.confirmDelete(e.detail));
        ...
        async submitForm() {
            ...
            const url = this.editMode 
                ? `{{ url('school/settings/admission-fee') }}/${this.feeId}` 
        ...
        async confirmDelete(fee) {
            window.dispatchEvent(new CustomEvent('open-confirm-modal', {
                detail: {
                    ...
                    message: `Are you sure you want to delete the admission fee for "${fee.name}"? This action cannot be undone.`,
                    callback: async () => {
                        try {
                            const response = await fetch(`{{ url('school/settings/admission-fee') }}/${fee.id}`, {
                                method: 'POST',
                                headers: {
                                    'Content-Type': 'application/json',
                                    'Accept': 'application/json',
