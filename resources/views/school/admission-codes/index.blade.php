    ...
    >
        <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            <div class="flex items-center justify-between mb-4 bg-blue-600 -mx-5 -mt-5 p-4 rounded-t-md">
                <h3 class="text-lg font-semibold text-white">Admission Code</h3>
                <button @click="closeAddModal()" class="text-white hover:text-gray-200">
                    <i class="fas fa-times"></i>
                </button>
            </div>
            
            <form action="{{ route('school.admission-codes.store') }}" method="POST" class="space-y-4">
                @csrf
                <div class="flex items-center space-x-4">
                    <label class="w-1/3 text-sm font-medium text-gray-700">Admission Code</label>
                    <input 
                        type="text" 
                        name="code" 
