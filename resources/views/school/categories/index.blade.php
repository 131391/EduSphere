...
@section('content')
    <div x-data="Object.assign(ajaxDataTable({
        fetchUrl: '{{ route('school.categories.fetch') }}',
        defaultSort: 'name',
    ...
    }), categoryManagement())" class="space-y-6" @close-modal.window="if ($event.detail === 'category-modal') { resetForm(); }">

        <!-- Statistics Cards -->
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <x-stat-card label="Total Categories" :value="$stats['total']" icon="fas fa-layer-group" color="indigo" alpine-text="stats.total" />
        </div>

        <!-- Header Section -->
        <x-page-header title="Categories" description="Manage student and staff classification categories" icon="fas fa-layer-group">
            ...
            <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
                <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    <div class="flex-1 flex flex-col md:flex-row md:items-center gap-4">
                        <h2 class="text-lg font-semibold text-gray-800 dark:text-white">Categories List</h2>
                        <x-table.search placeholder="Search categories..." />
                    </div>
            ...
            <div class="overflow-x-auto relative ajax-table-wrapper">
                ...
                <table class="w-full text-left border-collapse">
                    <thead class="bg-gray-50/50 dark:bg-gray-700/50 border-b border-gray-100 dark:border-gray-700">
                        <tr>
                            <x-table.sort-header column="name" label="Category Name" sort-var="sort" direction-var="direction" />
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-700 transition-opacity duration-150" x-show="hydrated" :class="loading && rows.length &gt; 0 ? 'opacity-50' : 'opacity-100'">
                        ...
                        <x-table.empty-state :colspan="3" icon="fas fa-layer-group" message="No categories found." />
                    </tbody>
                </table>
            </div>
        ...
        <!-- Add/Edit Modal -->
        <x-modal name="category-modal" alpineTitle="editMode ? 'Edit Category' : 'Create New Category'" maxWidth="2xl">
            <form @submit.prevent="submitForm()" method="POST" novalidate class="p-1">
                ...
                <div class="space-y-6">
                    <div class="space-y-2">
                        <label class="modal-label-premium">Category Name <span class="text-red-600 font-bold">*</span></label>
                ...
                <x-slot name="footer">
                    <button type="button" @click="$dispatch('close-modal', 'category-modal')" class="btn-premium-cancel px-10">
                        Cancel
                    </button>
                    <button type="button" @click="submitForm()" :disabled="submitting"
                        ...
                        <span x-text="editMode ? 'Update Changes' : 'Create Category'"></span>
                    </button>
                </x-slot>
            </form>
        </x-modal>
                ...
                return {
                    ...
                    async submitForm() {
                        ...
                        const url = this.editMode
                            ? `/school/categories/${this.categoryId}`
                            : '{{ route('school.categories.store') }}';
                    ...
                    confirmDelete(category) {
                        ...
                        window.dispatchEvent(new CustomEvent('open-confirm-modal', {
                            detail: {
                                ...
                                callback: async () => {
                                    try {
                                        const response = await fetch(`/school/categories/${category.id}`, {
                                            method: 'POST',
                                            headers: {
                                                'Content-Type': 'application/json',
                                                'Accept': 'application/json',
