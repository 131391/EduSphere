...
@section('content')
<div x-data="categoryManagement()">
    <!-- Header Section -->
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-4 mb-6">
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
                <h2 class="text-xl font-bold text-gray-800 dark:text-white">Categories</h2>
                <p class="text-sm text-gray-500 dark:text-gray-400">Manage student and staff classification categories</p>
            </div>
        ...
        $tableActions = [
            [
                ...
                'onclick' => function($row) {
                    ...
                    return "window.dispatchEvent(new CustomEvent('open-edit-category', { detail: JSON.parse(atob('$encoded')) }))";
                },
                'title' => 'Edit',
            ],
            [
                ...
                'onclick' => function($row) {
                    return "window.dispatchEvent(new CustomEvent('open-delete-category', { detail: { id: " . $row->id . ", name: '" . addslashes($row->name) . "' } }))";
                },
                'title' => 'Delete',
            ],
        ];
    ...
    <div x-on:open-edit-category.window="openEditModal($event.detail)" 
         x-on:open-delete-category.window="confirmDelete($event.detail)">
        <x-data-table 
    ...
    <x-modal name="category-modal" alpineTitle="editMode ? 'Edit Category' : 'Create New Category'" maxWidth="md">
        <form @submit.prevent="submitForm" method="POST" class="p-0" novalidate>
            ...
            <div class="px-8 py-8">
                <div>
                    <label class="block text-sm font-semibold text-gray-800 mb-1.5 ml-1">Category Name <span class="text-red-500">*</span></label>
            ...
            <div class="px-8 py-6 bg-gray-50/50 flex items-center justify-end gap-3 rounded-b-lg border-t border-gray-100">
                ...
                >
                    ...
                    <span x-text="editMode ? (submitting ? 'Updating' : 'Save Changes') : (submitting ? 'Creating' : 'Create Category')"></span>
                </button>
            </div>
        </form>
    </x-modal>
