...
@section('content')
    <div x-data="Object.assign(ajaxDataTable({
        fetchUrl: '{{ route('school.registration-codes.fetch') }}',
        defaultSort: 'code',
    ...
    }), registrationCodeManagement())" class="space-y-6" @close-modal.window="if ($event.detail === 'registration-code-modal') { resetForm(); }">

        <!-- Statistics Cards -->
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <x-stat-card label="Total Codes" :value="$stats['total']" icon="fas fa-fingerprint" color="indigo" alpine-text="stats.total" />
        </div>

        <!-- Header Section -->
        <x-page-header title="Registration Codes" description="Manage official registration codes for student tracking" icon="fas fa-fingerprint">
            ...
            <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
                <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    <div class="flex-1 flex flex-col md:flex-row md:items-center gap-4">
                        <h2 class="text-lg font-semibold text-gray-800 dark:text-white">Registration Codes List</h2>
                        <x-table.search placeholder="Search codes..." />
                    </div>
            ...
            <div class="overflow-x-auto relative ajax-table-wrapper">
                ...
                <table class="w-full text-left border-collapse">
                    <thead class="bg-gray-50/50 dark:bg-gray-700/50 border-b border-gray-100 dark:border-gray-700">
                        <tr>
                            <x-table.sort-header column="code" label="Registration Code" sort-var="sort" direction-var="direction" />
                            <th class="px-6 py-3 text-xs font-bold text-gray-500 uppercase tracking-wider">Description</th>
                            <x-table.sort-header column="created_at" label="Generated On" sort-var="sort" direction-var="direction" />
                            ...
                            <tr class="hover:bg-gray-50/50 dark:hover:bg-gray-800/50 transition-colors">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div class="text-sm font-bold text-gray-800 dark:text-gray-100 tracking-widest uppercase">{{ $row['code'] }}</div>
                                    </div>
                                </td>
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-700 transition-opacity duration-150" x-show="hydrated" :class="loading && rows.length &gt; 0 ? 'opacity-50' : 'opacity-100'">
                            ...
                            <tr class="hover:bg-gray-50/50 dark:hover:bg-gray-800/50 transition-colors">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div class="text-sm font-bold text-gray-800 dark:text-gray-100 tracking-widest uppercase" x-text="row.code"></div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400" x-text="row.description || 'N/A'"></td>
                        ...
                        <x-table.empty-state :colspan="4" icon="fas fa-fingerprint" message="No registration codes found." />
                    </tbody>
                </table>
            </div>
        ...
        <!-- Add/Edit Modal -->
        <x-modal name="registration-code-modal" alpineTitle="editMode ? 'Edit Registration Code' : 'New Registration Code'" maxWidth="2xl">
                ...
                <div class="space-y-6">
                    <div class="space-y-2">
                        ...
                        <div class="relative group">
                            <input type="text" name="code" x-model="formData.code" @input="clearError('code')"
                                ...
                                class="modal-input-premium pl-4 uppercase tracking-widest font-bold pr-10"
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea name="description" x-model="formData.description" @input="clearError('description')"
                            placeholder="Brief description about this code..."
                        ...
                        <template x-if="errors.description">
                            <p class="modal-error-message" x-text="errors.description[0]"></p>
                        </template>
                    </div>
                </div>

                <x-slot name="footer">
                    <button type="button" @click="$dispatch('close-modal', 'registration-code-modal')" class="btn-premium-cancel px-10">
                        Cancel
                    </button>
                    <button type="button" @click="submitForm()" :disabled="submitting"
                        ...
                        <span x-text="editMode ? 'Update Changes' : 'Save Code'"></span>
                    </button>
                </x-slot>
            </form>
        </x-modal>
                ...
                return {
                    ...
                    async submitForm() {
                        ...
                        const url = this.editMode
                            ? `/school/registration-codes/${this.codeId}`
                            : '{{ route('school.registration-codes.store') }}';
                    ...
                    confirmDelete(item) {
                        ...
                        window.dispatchEvent(new CustomEvent('open-confirm-modal', {
                            detail: {
                                ...
                                message: `Are you sure you want to delete the registration code "${item.code}"? This action cannot be undone.`,
                                callback: async () => {
                                    try {
                                        const response = await fetch(`/school/registration-codes/${item.id}`, {
                                            method: 'POST',
                                            headers: {
                                                'Content-Type': 'application/json',
                                                'Accept': 'application/json',
