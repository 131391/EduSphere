...
function registrationManagement() {
    return {
        ...
        validateCurrentStep() {
            const e = {};
            // Clear only this step's errors before re-validating
            const stepFields = {
        ...
        updateFee() {
            ...
            // Use !== undefined so zero-fee classes (value '0') still update correctly
        ...
        async fetchEnquiryData() {
            ...
            if (!id) return;
            try {
                const res = await fetch(`/school/student-registrations/enquiry/${id}`);
                ...
                // Clear errors for every field that was just populated
                const filledFields = [
        ...
        async submitForm() {
            ...
            Object.entries(this.formData).forEach(([key, value]) => {
                if (locationFields.has(key)) return; // read from DOM below
            ...
            try {
                const response = await fetch("{{ route('school.student-registrations.store') }}", {
                    method: 'POST',
                    headers: {
        ...
        handleValidationErrors(errors) {
            ...
            // Evaluate step3 fields at call time (not at map creation time)
