...
@section('content')
    <div x-data="Object.assign(ajaxDataTable({
            fetchUrl: '{{ route('school.admission.fetch') }}',
        ...
        <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
            <x-stat-card label="Total Registrations" :value="$stats['total_registration']" icon="fas fa-file-alt"
                color="blue" alpine-text="stats.total_registration" />
            <x-stat-card label="Confirmed Admitted" :value="$stats['admission_done']" icon="fas fa-user-check"
                color="emerald" alpine-text="stats.admission_done" />
            <x-stat-card label="Pending Approval" :value="$stats['pending_registration']" icon="fas fa-clock" color="amber"
                alpine-text="stats.pending_registration" />
            <x-stat-card label="Cancelled Nodes" :value="$stats['cancelled_registration']" icon="fas fa-times-circle"
                color="rose" alpine-text="stats.cancelled_registration" />
            <x-stat-card label="Source Leads" :value="$stats['total_enquiry']" icon="fas fa-question-circle" color="purple"
                alpine-text="stats.total_enquiry" />
        </div>

        <!-- Header Section -->
        <x-page-header title="Admission Registry"
            description="Manage confirmed student admissions and academic cluster assignments." icon="fas fa-user-graduate">
            <a href="{{ route('school.admission.create') }}"
            ...
            <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
                <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    ...
                    <div class="flex items-center gap-3">
                        <x-table.filter-select model="filters.class_id"
                            action="applyFilter('class_id', $event.target.value)" placeholder="Class"
                            :options="$classes->pluck('name', 'id')->toArray()" />

                        <x-table.filter-select model="filters.section_id"
                            action="applyFilter('section_id', $event.target.value)" placeholder="Section"
            ...
            <div class="overflow-x-auto relative ajax-table-wrapper">
                ...
                <table class="w-full text-left border-collapse">
                    <thead class="bg-gray-50/50 dark:bg-gray-700/50 border-b border-gray-100 dark:border-gray-700">
                        <tr>
                            <x-table.sort-header column="admission_no" label="Admission No" sort-var="sort"
                                direction-var="direction" />
                            <x-table.sort-header column="first_name" label="Student Name" sort-var="sort"
                                direction-var="direction" />
                            <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Class &
                                Section</th>
                            <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Parent
                                & Contact</th>
                            <th class="px-6 py-3 text-center text-xs font-bold text-gray-500 uppercase tracking-wider">
                                Status</th>
                            <x-table.sort-header column="admission_date" label="Admission Date" sort-var="sort"
                                direction-var="direction" />
                        ...
                        @foreach($initialData['rows'] as $row)
                            <tr class="hover:bg-gray-50/50 dark:hover:bg-gray-800/50 transition-colors group">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span
                                        class="text-[10px] font-black text-teal-600 bg-teal-50 px-2.5 py-1.5 rounded-lg border border-teal-100 uppercase tabular-nums tracking-widest">#{{ $row['admission_no'] }}</span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center gap-3">
                                        <div
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex flex-col">
                                        <span
                                            class="text-xs font-bold text-gray-700 dark:text-gray-200">{{ $row['class_name'] }}</span>
                                        <span class="text-[10px] font-medium text-teal-500 mt-0.5">Section
                                            {{ $row['section_name'] }}</span>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="space-y-1">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <span
                                        class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full border bg-emerald-50 text-emerald-700 border-emerald-100 text-[10px] font-bold uppercase tracking-wider shadow-sm">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        <a href="{{ route('school.admission.pdf', $row['id']) }}"
                                            class="w-8 h-8 rounded-lg bg-rose-50 text-rose-600 flex items-center justify-center hover:bg-rose-100 transition-colors"
                                            title="Download PDF"><i class="fas fa-file-pdf text-xs"></i></a>
                                        <a href="{{ route('school.admission.show', $row['id']) }}"
                                            class="w-8 h-8 rounded-lg bg-blue-50 text-blue-600 flex items-center justify-center hover:bg-blue-100 transition-colors"
                                            title="View"><i class="fas fa-eye text-xs"></i></a>
                                        <a href="{{ route('school.admission.edit', $row['id']) }}"
                                            class="w-8 h-8 rounded-lg bg-teal-50 text-teal-600 flex items-center justify-center hover:bg-teal-100 transition-colors"
                                            title="Edit"><i class="fas fa-edit text-xs"></i></a>
                                        <button
                                            @click="quickAction(`/school/admission/${row['id']}`, 'Delete Admission', 'DELETE')"
                                            class="w-8 h-8 rounded-lg bg-rose-50 text-rose-600 flex items-center justify-center hover:bg-rose-100 transition-colors"
                                            title="Delete"><i class="fas fa-trash-alt text-xs"></i></button>
                                    </div>
                                </td>
                            </tr>
                        @endforeach
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-700 transition-opacity duration-150" x-cloak
                        ...
                        <template x-for="row in rows" :key="row.id">
                            <tr class="hover:bg-gray-50/50 dark:hover:bg-gray-800/50 transition-colors group">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span
                                        class="text-[10px] font-black text-teal-600 bg-teal-50 px-2.5 py-1.5 rounded-lg border border-teal-100 uppercase tabular-nums tracking-widest"
                                        x-text="'#' + row.admission_no"></span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center gap-3">
                                        <div
                                        ...
                                        <div>
                                            <div class="text-sm font-bold text-gray-800 dark:text-gray-100"
                                                x-text="row.full_name"></div>
                                            <div class="text-[10px] font-medium text-gray-400"
                                                x-text="'#' + row.admission_no"></div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex flex-col">
                                        <span class="text-xs font-bold text-gray-700 dark:text-gray-200"
                                            x-text="row.class_name"></span>
                                        <span class="text-[10px] font-medium text-teal-500 mt-0.5"
                                            x-text="'Section ' + row.section_name"></span>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="space-y-1">
                                        <div class="text-xs font-bold text-gray-700 dark:text-gray-200"
                                            x-text="row.father_name"></div>
                                        <div
                                            class="flex items-center gap-1.5 text-[10px] text-gray-400 tabular-nums font-medium">
                                            <i class="fas fa-phone-alt text-[8px] text-teal-500"></i>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        <a :href="`/school/admission/${row.id}/pdf`"
                                            class="w-8 h-8 rounded-lg bg-rose-50 text-rose-600 flex items-center justify-center hover:bg-rose-100 transition-colors"
                                            title="Download PDF"><i class="fas fa-file-pdf text-xs"></i></a>
                                        <a :href="`/school/admission/${row.id}`"
                                            class="w-8 h-8 rounded-lg bg-blue-50 text-blue-600 flex items-center justify-center hover:bg-blue-100 transition-colors"
                                            title="View"><i class="fas fa-eye text-xs"></i></a>
                                        <a :href="`/school/admission/${row.id}/edit`"
                                            class="w-8 h-8 rounded-lg bg-teal-50 text-teal-600 flex items-center justify-center hover:bg-teal-100 transition-colors"
                                            title="Edit"><i class="fas fa-edit text-xs"></i></a>
                                        <button
                                            @click="quickAction(`/school/admission/${row.id}`, 'Delete Admission', 'DELETE')"
                        ...
                        <x-table.empty-state :colspan="7" icon="fas fa-user-graduate"
                            message="No admissions found matching your criteria." />
                    </tbody>
                </table>
            </div>
