        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
                ...
                <div class="flex flex-col items-center">
                    ...
                    <input type="hidden" name="father_photo_path" id="father_photo_path" value="{{ old('father_photo_path') }}">
                </div>
            </div>
            ...
            <div>
                ...
                <div class="flex flex-col items-center">
                    ...
                    <input type="file" name="mother_photo" accept="image/*" 
                           ...
                           class="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-teal-50 file:text-teal-700 hover:file:bg-teal-100">
                    <input type="hidden" name="mother_photo_path" id="mother_photo_path" value="{{ old('mother_photo_path') }}">
                </div>
            </div>
            ...
            <div>
                ...
                <div class="flex flex-col items-center">
                    <div class="w-32 h-32 bg-gray-200 dark:bg-gray-600 rounded-lg mb-2 flex items-center justify-center overflow-hidden relative">
                        <img id="student-photo-preview" src="#" alt="Student's Photo" class="hidden w-full h-full object-cover">
                    ...
                    <input type="file" name="student_photo" accept="image/*" 
                           ...
                           class="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-teal-50 file:text-teal-700 hover:file:bg-teal-100">
                    <input type="hidden" name="student_photo_path" id="student_photo_path" value="{{ old('student_photo_path') }}">
                </div>
            </div>
        </div>
    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        const photos = [
            { path: 'father_photo_path', preview: 'father-photo-preview', icon: 'father-photo-icon', remove: 'father-photo-remove' },
            { path: 'mother_photo_path', preview: 'mother-photo-preview', icon: 'mother-photo-icon', remove: 'mother-photo-remove' },
            { path: 'student_photo_path', preview: 'student-photo-preview', icon: 'student-photo-icon', remove: 'student-photo-remove' }
        ];
        
        photos.forEach(photo => {
            const hiddenInput = document.getElementById(photo.path);
            if (hiddenInput && hiddenInput.value) {
                if (typeof loadImagePreview === 'function') {
                    loadImagePreview(hiddenInput.value, photo.preview, photo.icon, photo.remove);
                }
            }
        });
    });
...
function previewImage(event, previewId, iconId, removeBtnId) {
    ...
    const inputName = event.target.name; // e.g., 'father_photo'
    if (file) {
