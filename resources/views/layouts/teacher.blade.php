...
<body class="bg-gray-100 dark:bg-gray-900 h-screen overflow-hidden transition-colors">
                ...
                <div class="flex items-center justify-between px-4 sm:px-6 py-4">
                    ...
                    <div class="flex items-center space-x-2 sm:space-x-4" x-data="{
                        ...
                        <button @click="toggleDarkMode()"
                            ...
                            <i class="text-xl far fa-moon" x-show="false" style="display:inline-block" id="teacher-dark-icon"></i>
                        </button>
                        <script>if(localStorage.getItem('darkMode')==='true'||(!('darkMode'in localStorage)&&window.matchMedia('(prefers-color-scheme: dark)').matches)){var _d=document.getElementById('teacher-dark-icon');if(_d){_d.className='text-xl fas fa-sun';}}</script>
    ...
    <script>
        ...
        function initSelect2($select) {
            ...
            try {
                $select.select2({ placeholder: function() { return $(this).data('placeholder') || 'Select an option'; }, allowClear: false, width: '100%' });
                return true;
        ...
        $(document).ready(function() {
            ...
            const obs = new MutationObserver(m => {
                let s = [];
                m.forEach(mu => { if (mu.addedNodes.length) $(mu.addedNodes).find('select').each(function() { const $s=$(this); if (!$s.hasClass('select2-hidden-accessible') && !$s.hasClass('no-select2') && $s.closest('[x-cloak]').length===0) s.push(this); }); });
                if (s.length) { clearTimeout(t); t = setTimeout(() => $(s).each(function() { initSelect2($(this)); }), 50); }
            });
