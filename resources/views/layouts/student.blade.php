    ...
    <aside class="fixed inset-y-0 left-0 z-50 bg-[#1a237e] text-white flex flex-col transform transition-all duration-300 ease-in-out lg:translate-x-0 lg:static lg:inset-0"
        ...
        <div class="p-4 border-b border-[#283593] flex-shrink-0 relative">
            ...
            <div x-show="!sidebarCollapsed" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100">
                <h2 class="text-xs font-bold text-center leading-tight">STUDENT PORTAL</h2>
        ...
        <nav class="flex-1 overflow-y-auto py-4 px-2 space-y-1">
            @php
                $navItems = [
                    ['route' => 'student.dashboard', 'icon' => 'fa-home', 'label' => 'Dashboard'],
                    ['route' => 'student.results.index', 'icon' => 'fa-trophy', 'label' => 'My Results'],
                    ['route' => 'student.attendance.index', 'icon' => 'fa-calendar-check', 'label' => 'Attendance'],
                    ['route' => 'student.fees.index', 'icon' => 'fa-receipt', 'label' => 'Fee Details'],
                    ['route' => 'student.timetable.index', 'icon' => 'fa-clock', 'label' => 'Timetable'],
                ];
            @endphp
        ...
        <div class="border-t border-[#283593] p-3 flex-shrink-0">
            <div class="flex items-center gap-3">
                ...
                <div x-show="!sidebarCollapsed" x-transition class="flex-1 min-w-0">
                    <p class="text-xs font-medium truncate">{{ Auth::user()->name ?? 'Student' }}</p>
                    <p class="text-xs text-indigo-300 truncate">Student</p>
                </div>
            </div>
