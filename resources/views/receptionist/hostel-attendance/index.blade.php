...
@section('title', 'Hostel Attendance - Receptionist')
...
<div class="space-y-6" x-data="hostelAttendanceManagement()">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden relative"
        ...
        {{-- ── Toolbar (visible when students loaded) ── --}}
        ...
        {{-- ── Footer Save Bar (visible when students loaded) ── --}}
        <div x-show="students.length > 0" x-cloak
...
document.addEventListener('alpine:init', () => {
    Alpine.data('hostelAttendanceManagement', () => ({
        ...
        init() {
            // Alpine calls init() automatically — no x-init needed.
        },
        ...
        async loadStudents() {
            ...
            try {
                ...
                if (data.success) {
                    this.students = data.students.map(s => ({ ...s, id: String(s.id), remarks: '' }));
                    this.checkedStudents = this.students.map(s => s.id);
                } else {
                    this.students = [];
                    this.checkedStudents = [];
        ...
        updateStatus(studentId, isChecked) {
            ...
            if (isChecked) {
                if (!this.checkedStudents.includes(id)) this.checkedStudents.push(id);
            } else {
                this.checkedStudents = this.checkedStudents.filter(i => i !== id);
            }
        },
            ...
            try {
                const payload = {
                    ...
                    students: this.students.map(s => ({
                        student_id: parseInt(s.id, 10), // send integer — DB pluck returns int, array_diff is type-sensitive
                        is_present: this.checkedStudents.includes(s.id),
                        remarks: s.remarks || ''
                    }))
                };
                ...
                if (response.ok && result.success) {
                    // Wait for the toast to finish before navigating — prevents the double-blink
                    // caused by reload() firing while the DOM is still updating.
                    if (window.Toast) {
