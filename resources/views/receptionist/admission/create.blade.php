...
function admissionManagement() {
    return {
        ...
        init() {
            if (this.formData.class_id) this.loadClassData(this.formData.class_id);
        },
        ...
        async fetchRegistrationData() {
            ...
            if (!id) return;
            try {
                const res = await fetch(`{{ url('receptionist/admission/registration') }}/${id}`);
                ...
                ['student_photo','father_photo','mother_photo','student_signature','father_signature','mother_signature'].forEach(f => {
                    if (d[f]) { this.formData[`${f}_path`] = d[f]; this.previews[f] = `/storage/${d[f]}`; }
                });
        ...
        handlePhotoUpload(event, field) {
            ...
            if (file) { this.previews[field] = URL.createObjectURL(file); this.formData[`${field}_path`] = ''; this.clearError(field); }
        },
        ...
        async submitForm() {
            ...
            try {
                const response = await fetch("{{ route('receptionist.admission.store') }}", {
                    method: 'POST',
                    headers: {
