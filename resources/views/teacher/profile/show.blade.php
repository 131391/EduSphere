...
<div class="space-y-6 max-w-5xl">
        ...
        <div class="flex flex-col md:flex-row md:items-center gap-5">
            ...
            @else
                <div class="w-24 h-24 rounded-2xl bg-indigo-100 dark:bg-indigo-900/30 flex items-center justify-center text-3xl font-bold text-indigo-600 dark:text-indigo-300">
                    {{ strtoupper(substr($teacher->first_name ?? 'T', 0, 1)) }}{{ strtoupper(substr($teacher->last_name ?? '', 0, 1)) }}
                </div>
            @endif
            <div class="flex-1">
                <h2 class="text-2xl font-bold text-gray-800 dark:text-white">{{ $teacher->full_name ?: $user->name }}</h2>
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
                    {{ $user->email }}
                    @if($teacher->employee_id)
                        &middot; Employee ID: <span class="font-mono">{{ $teacher->employee_id }}</span>
                    @endif
                </p>
                <div class="mt-3 flex flex-wrap gap-2">
                    ...
                    @if($teacher->qualification)
                        <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-emerald-50 text-emerald-700">
                            <i class="fas fa-graduation-cap text-[10px]"></i> {{ $teacher->qualification }}
                        </span>
                    @endif
                    @if($teacher->joining_date)
                        <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-amber-50 text-amber-700">
                            <i class="fas fa-calendar text-[10px]"></i> Joined {{ $teacher->joining_date->format('M Y') }}
                        </span>
                    @endif
                </div>
            </div>
    ...
    <form method="POST" action="{{ route('teacher.profile.update') }}" enctype="multipart/form-data"
        ...
        <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div>
                <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">Date of Birth</label>
                <input type="date" name="date_of_birth" value="{{ old('date_of_birth', optional($teacher->date_of_birth)->toDateString()) }}" max="{{ now()->subYear()->toDateString() }}"
                       class="w-full h-10 px-3 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg text-sm text-gray-700 dark:text-gray-200 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                @error('date_of_birth')<p class="text-xs text-rose-600 mt-1">{{ $message }}</p>@enderror
            </div>

            <div>
                <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">Qualification</label>
                <input type="text" name="qualification" value="{{ old('qualification', $teacher->qualification) }}" placeholder="e.g. M.Sc. Mathematics"
                       class="w-full h-10 px-3 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg text-sm text-gray-700 dark:text-gray-200 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                @error('qualification')<p class="text-xs text-rose-600 mt-1">{{ $message }}</p>@enderror
            </div>

            <div class="md:col-span-2">
                <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">Address</label>
                <textarea name="address" rows="3"
                          class="w-full px-3 py-2 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg text-sm text-gray-700 dark:text-gray-200 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">{{ old('address', $teacher->address) }}</textarea>
